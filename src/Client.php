...
class Client
{
    ...
    /**
     * Refresh token returned by the service provider after a successful authentication
     *
     * @var string
     *
     * @access private
    ...
    /**
     * Automatically decode json response
     *
     * @var bool
     *
     * @access private
     */
    private $_jsonAutoDecode = false;

    /**
     * Called when new token generated by refresh token
     *
     * @var callable
     *
     * @access private
     */
    private $_autoUpdateAccessTokenCallback;
    ...
    public function credentialsFlow($username, $password)
    {
        ...
        $response = $this->_request($this->getAccessTokenUrl(), $options);
    ...
    /**
     * Set autodecode JSON option
     *
     * @param bool $state Autodecode json state
     *
     * @access public
     */
    public function setJSONAutoDecode($state) {
        $this->_jsonAutoDecode = $state;
        return $this;
    }

    /**
     * Set update access token callback
     *
     * @param callable $callback
     *
     * @access public
     */
    public function setAutoUpdateAccessTokenCallback(callable $callback) {
        $this->_autoUpdateAccessTokenCallback = $callback;
        return $this;
    }
    ...
    protected function _getAccessToken($postData, $curlOptions = array())
    {
        $options = array(CURLOPT_POST => true, CURLOPT_POSTFIELDS => $postData);
        $options += $curlOptions;
        $response = $this->_doRequest($this->getAccessTokenUrl(), $options);
        if(!is_array($response)) {
            $response = json_decode($response);
        }
    ...
    /**
     * Wrapper for _doRequest
     */
    protected function _request($url, $curlOptions = array())
    {
        try {
            return $this->_doRequest($url, $curlOptions);
        }
        catch(InvalidHttpResponseCodeException $ex) {
            // only auto refresh token if user set callback
            if($ex->getHttpCode() == 401 && $this->_refreshToken && $this->_autoUpdateAccessTokenCallback) {
                // try to refresh token
                $this->setAccessToken(null);
                $newAccessToken = $this->accessTokenRefresh($this->_refreshToken);
                call_user_func($this->_autoUpdateAccessTokenCallback, $newAccessToken);
                $this->setAccessToken($newAccessToken["access_token"]);
                $this->setRefreshToken($newAccessToken["refresh_token"]);
                // try request again
                return $this->_doRequest($url, $curlOptions);
            }
            else {
     ...
     * @access protected
     */
    protected function _doRequest($url, $curlOptions = array()) {
        $ch = curl_init($url);
        ...
        if ($this->_validResponseCode($this->_lastHttpResponseCode)) {
            $result = $this->_lastHttpResponseBody;
            if($this->_jsonAutoDecode) {
